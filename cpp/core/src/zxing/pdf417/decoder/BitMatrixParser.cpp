...
namespace zxing {
    namespace pdf417 {
        ...
        int BitMatrixParser::MAX_ROW_DIFFERENCE = 6;
        int BitMatrixParser::MAX_ROWS = 90;
        int BitMatrixParser::MAX_CW_CAPACITY = 929;
        int BitMatrixParser::MODULES_IN_SYMBOL = 17;
        
        
        BitMatrixParser::BitMatrixParser(Ref<BitMatrix> bitMatrix) :
            bitMatrix_(bitMatrix), rows(0), leftColumnECData(0), rightColumnECData(0), eraseCount(0), ecLevel(-1) {
            erasures = new int[MAX_CW_CAPACITY];    
        }
        
        int BitMatrixParser::SYMBOL_TABLE[] = {0x1025e, 0x1027a, 0x1029e,
         ...
         */
        const int BitMatrixParser::CODEWORD_TABLE[] = {2627, 1819, 2622, 2621, 1813,
...
//        void trimArray(ArrayRef<int> array, int size) {
//            if (size > 0) {
//                int[] a = new int[size];
...
//        }
        ...
        int BitMatrixParser::getCodewordByIndex(int idx){
            long cw = BitMatrixParser::CODEWORD_TABLE[idx] - 1;
            cw %= 929;
            return (int) cw;
        }
        ...
        ArrayRef<int> BitMatrixParser::readCodewords() {
            ...
            ArrayRef<int> rowCounters(width);
            ArrayRef<int> codewords(MAX_CW_CAPACITY);
            int next = 0;
            ...
            for (int i = 1; i < height; i++) {
                if (rowNumber >= MAX_ROWS) {
                    ...
                    // TODO Maybe return error code
                        throw ReaderException("maximum rows exceeded");
                }
                int rowDifference = 0;
                ...
                for (int j = 0; j < width; j++) {
                    ...
                    // previous line.
                    if (bitMatrix_->get(j, i) != bitMatrix_->get(j, i - 1)) {
                        rowDifference++;
                    }
                }
                if (rowDifference <= moduleWidth * MAX_ROW_DIFFERENCE) {
                    ...
                    if (rowInProgress) {
                        // Process Row
                        next = processRow(rowCounters, width, rowNumber, rowHeight, codewords, MAX_CW_CAPACITY, next);
                        if (next == -1) {
                            // Something is wrong, since we have exceeded
                            // the maximum columns in the specification.
                            // TODO Maybe return error code
                            throw ReaderException("maximum columns exceeded");
                        }
            ...
            ArrayRef<int> result(next);
            for(int i=0; i<next; i++){
                result[i] = codewords[i];
            }
            return result;
        }
        
        int BitMatrixParser::processRow(ArrayRef<int> rowCounters, int rowCountersSize, int rowNumber, int rowHeight, ArrayRef<int> codewords, int codewordsSize, int next) {
            int width = bitMatrix_->getWidth();
            int columnNumber = 0;
            long symbol = 0;
            
            for (int i = 0; i < width; i += MODULES_IN_SYMBOL) {
                if (i + MODULES_IN_SYMBOL > rowCountersSize) {
                    throw ReaderException("Misdecode of symbol");
                }
        ...
        int BitMatrixParser::getCodeword(long symbol) {
            long sym = symbol;
            sym &= 0x3ffff;
            ...
            if (i == -1) {
                ...
                long cw = BitMatrixParser::CODEWORD_TABLE[i] - 1;
                cw %= 929;
                return (int) cw;
            }
        }
        ...
        int BitMatrixParser::findCodewordIndex(long symbol) {
            int first = 0;
            int upto = sizeof(BitMatrixParser::SYMBOL_TABLE)/sizeof(BitMatrixParser::SYMBOL_TABLE[0]);
            while (first < upto) {
